...
class SquirtleBulletShooter {
  ...
  keyDownHandler(e) {
    ...
    if (e.keyCode === 90 && this.bullets.length < 3) {
      ...
      this.bullets.push(
        new SquirtleBullet(this.canvas, this.ctx, this.Squirtle)
      );
    }
  }
          ...
          for (let i = 0; i < 6; i++) {
            for (let j = 0; j < 2; j++) {
              if (this.Butterfrees.butterfrees[i][j].health > 0) {
                ...
                if (
                  bullet.x > butterfree.x - 25 &&
                  bullet.x < butterfree.x + 35 &&
                  bullet.y > butterfree.y - 15 &&
                  bullet.y < butterfree.y + 15
                ) {
                  butterfree.health -= 1;
                  bullet.health -= 1;
          ...
          for (let k = 0; k < 5; k++) {
            for (let l = 0; l < 2; l++) {
              if (this.Beedrills.beedrills[k][l].health > 0) {
                ...
                if (
                  bullet.x > beedrill.x - 25 &&
                  bullet.x < beedrill.x + 15 &&
                  bullet.y > beedrill.y - 15 &&
                  bullet.y < beedrill.y + 15
                ) {
                  beedrill.health -= 1;
                  bullet.health -= 1;
