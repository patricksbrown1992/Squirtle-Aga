...
class Game {
  constructor(
    ...
    this.EnemyBulletShooter = new EnemyBulletShooter(
      this.ctx,
      this.canvas,
      this.Beedrill,
      this.Butterfree,
    ...
    document.addEventListener(
      "keydown",
      this.SquirtleBulletShooter.keyDownHandler,
      false
    );
  ...
  keyHandler(e) {
    ...
    if (e.keyCode === 72 && this.state == STATUS.GAMEOVER) {
      this.Squirtle = new this.newSquirtle(
        this.canvas,
        this.ctx,
        this.squirtlePhoto
      );
      this.Beedrill = new this.newBeedrill(
        this.canvas,
        this.ctx,
        this.beedrillPhoto
      );
      this.Butterfree = new this.newButterfree(
        this.canvas,
        this.ctx,
        this.butterfreePhoto
      );
      this.SquirtleBulletShooter = new this.newSquirtleBulletShooter(
        this.ctx,
        this.canvas,
        this.Squirtle,
        this.Butterfree,
      ...
      this.EnemyBulletShooter = new this.newEnemyBulletShooter(
        this.ctx,
        this.canvas,
        this.Beedrill,
        this.Butterfree,
    ...
    if (
      (e.keyCode === 69 && this.state == STATUS.WIN) ||
      (this.state == STATUS.WIN && e.keyCode == 67)
    ) {
      ...
      if (this.Squirtle.health <= 0) {
        ...
        if (count == 0) {
          ...
          this.SquirtleBulletShooter.bullets.forEach((bullet) => {
            ...
            this.SquirtleBulletShooter.bullets = this.SquirtleBulletShooter.bullets.filter(
              (bullet) => bullet.y > 0
            );
            this.SquirtleBulletShooter.bullets = this.SquirtleBulletShooter.bullets.filter(
              (bullet) => bullet.health > 0
            );
          });
          ...
          this.EnemyBulletShooter.bullets.forEach((bullet) => {
            ...
            this.EnemyBulletShooter.bullets = this.EnemyBulletShooter.bullets.filter(
              (bullet) => bullet.y < 540
            );
          });
        }
